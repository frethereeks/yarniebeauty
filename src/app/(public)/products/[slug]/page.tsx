...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!product || product.status !== "Available") {
    return {
      title: "Product Not Found | Yarniebeauty",
    };
  }

  // Find the primary preview image or grab the first available fallback
  const images = product.images || [];
  const primaryImage = images.find((i) => i.isPrimary) ?? images[0];
  const imageUrl = primaryImage?.secureUrl || "/brand/logo-old.jpg";

  const pageUrl = `${process.env.NEXTAUTH_URL}/products/${slug}`;
  const fullTitle = `${product.name} — Yarniebeauty`;
  const plainDescription = product.description || "Handcrafted yarn, made-to-order crochet pieces, and custom designs by Yarniebeauty.";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `View ${product.name} on Yarniebeauty`,
        },
      ],
      locale: "en_US",
    },
