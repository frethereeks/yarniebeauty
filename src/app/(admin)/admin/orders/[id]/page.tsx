    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-line">
            <div className="px-6 py-4 border-b border-line">
              <h2 className="text-eyebrow text-text-on-cream-muted">Products in this order</h2>
            </div>
            ...
            <div className="px-6 py-4 border-t border-line flex justify-between font-semibold">
              <span>Total</span>
              <span>{currency.format(Number(order.totalPrice))}</span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-line p-6">
            <h2 className="text-eyebrow text-text-on-cream-muted mb-4">Customer</h2>
            <dl className="space-y-2 text-sm">
              <Row label="Name" value={`${order.user.firstname} ${order.user.lastname}`} />
          ...
          <div className="bg-white border border-line p-6">
            ...
            <dl className="space-y-2 text-sm">
              <Row label="Recipient" value={order.fullname} />
          ...
          {/* Per-order payment info — available to regular admins */}
          <div className="bg-white border border-line p-6">
            <h2 className="text-eyebrow text-text-on-cream-muted mb-4">Payment</h2>
            <dl className="space-y-2 text-sm">
              <Row label="Status" value={order.payment?.status ?? "—"} />
              <Row label="Amount" value={currency.format(Number(order.totalPrice))} />
              {order.payment?.channel && <Row label="Channel" value={order.payment.channel} />}
              {order.payment?.paidAt && <Row label="Paid at" value={dateFmt.format(order.payment.paidAt)} />}
              {order.payment?.reference && <Row label="Reference" value={order.payment.reference} />}
            </dl>
          </div>
        </div>
      </div>
